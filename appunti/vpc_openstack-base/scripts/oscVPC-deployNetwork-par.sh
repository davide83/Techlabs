...
# Create the regional Subnet 10-42-64-0_24
VPC_SUBNET_NAME=pnSbnt-$VPC_SEGMENT_ID-$VPC_REGION_NAME-10-42-64-0_24
openstack subnet create \
    --description "Techlabs' VPC (vlan id $VPC_SEGMENT_ID) Private Subnet 10.42.64.0/24 in $VPC_REGION_NAME region" \
    --network $VPC_NET_NAME \
    --subnet-range 10.42.64.0/24 \
    --dhcp \
    --allocation-pool start=10.42.64.64,end=10.42.64.254 \
    --gateway 10.42.64.1 \
    --dns-nameserver 213.186.33.99 \
    --dns-nameserver 1.1.1.1 \
    --dns-nameserver 8.8.8.8 \
    --host-route destination=10.42.16.0/24,gateway=10.42.16.1 \
    --host-route destination=10.42.32.0/24,gateway=10.42.32.1 \
